    ...
    if (!isSameMonthAndYear) {
      ...
      if (monthlyBonus) {
        ...
        if (!updatedUser) {
          ...
          return NextResponse.json({ bonusAwarded: false, message: "Monthly bonus already awarded" });
        }

        const newRewardPoints = (updatedUser.confirmedPoints || 0) + (updatedUser.unconfirmedPoints || 0);
        await User.findByIdAndUpdate(updatedUser._id, { $set: { rewardPoints: newRewardPoints } });
